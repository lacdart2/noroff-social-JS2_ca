...
export function renderNavbar(containerId = "navbarContainer") {
  ...
  // Prevent multiple intervals stacking up
  ...
  /*   container.innerHTML = `
  <nav class="bg-gray-900 text-gray-200 shadow-lg p-4 flex justify-between items-center m-0">
  ...
  <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-home"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l-2 0l9 -9l9 9l-2 0" /><path d="M5 12v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-7" /><path d="M9 21v-6a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2v6" /></svg>
        ...
        <div class="flex items-center gap-6">
          <a href="/pages/profile/index.html" class="flex items-center gap-1 text-gray-400 hover:text-blue-400">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" /></svg>
            <span>Profile</span>
          </a>
  
          <a href="/pages/posts/index.html" class="flex items-center gap-1 text-gray-400 hover:text-blue-400">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-article" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none">
              <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
              <path d="M3 4m0 2a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v14a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2z" />
              <path d="M7 8h10" />
              <path d="M7 12h10" />
              <path d="M7 16h10" />
            </svg>
        ...
        <div class="relative">
            <button id="notificationBtn" class="relative">
              <span id="notificationCount"
                class="absolute -top-1 -right-1 bg-red-500 text-white rounded-full text-xs px-1 leading-none hidden z-10">
                0
              </span>
            ...
            <div id="notificationList"
              class="hidden flex flex-col absolute right-0 top-full mt-2 w-80
                    bg-white border shadow-xl rounded-lg text-sm max-h-96 overflow-auto
                    z-50 opacity-0 pointer-events-none transition-all duration-300">
            </div>
  
  
        </div>
        <div class="flex items-center gap-3">
          <span id="navUserName" class="text-gray-600 text-sm font-medium"></span>
...
`;
  ...
  updateBellBadgeAndIcon(); // ✅ So it runs immediately on page load too

  //  2. Add the click event listener AFTER injecting the HTML
  ...
  bell.addEventListener("click", async (e) => {
    ...
    // Toggle visibility (render function no longer hides the list)
  ...
  window.addEventListener("click", (e) => {
    ...
    // (keeps your existing notification close logic)
  });
  ...
  window.notificationInterval = setInterval(async () => {
    await checkNotifications(username); // computes fresh count only
    // (optional) you can avoid re-rendering here; we only update the badge/icon:
    updateBellBadgeAndIcon();
  }, 8000);
